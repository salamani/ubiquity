...
func (s *remoteClient) CreateVolume(createVolumeRequest resources.CreateVolumeRequest) error {
	defer s.logger.Trace(logs.DEBUG, logs.Args{logs.NameValue{Name: "request_id", Value: createVolumeRequest.Id}})()
	...
	response, err := utils.HttpExecute(s.httpClient, "POST", createRemoteURL, createVolumeRequest, createVolumeRequest.Id)
	if err != nil {
		return s.logger.ErrorRet(err, "utils.HttpExecute failed")
	}
